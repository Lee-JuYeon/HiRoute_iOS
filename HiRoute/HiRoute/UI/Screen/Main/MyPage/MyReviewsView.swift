            ...
            if placeVM.myReviews.isEmpty {
                ...
                ScrollView(.vertical) {
                    LazyVStack(spacing: 0) {
                        ForEach(placeVM.myReviews, id: \.reviewUid) { reviewModel in
                            ReviewCell(
                            ...
                            .onAppear {
                                if reviewModel.reviewUid == placeVM.myReviews.last?.reviewUid {
                                    placeVM.loadMoreMyReviews()
                                }
                            }
                        }

                        if placeVM.isLoadingMoreMyReviews {
                            HStack {
                                Spacer()
                                ProgressView()
                                Spacer()
