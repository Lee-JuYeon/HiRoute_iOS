...
struct PlaceEntityMapper {
    static func toModel(_ entity: PlaceEntity?, fullData: Bool = true) -> PlaceModel? {
        ...
        // 🖼 썸네일 이미지 변환
        let thumbnailImage: ImageModel? = entity.thumbnailImageURL.map { url in
            ImageModel(
                id: UUID().uuidString,
                imageUrl: url,
    ...
    static func toEntity(_ model: PlaceModel, context: NSManagedObjectContext) -> PlaceEntity {
        ...
        entity.thumbnailImageURL = model.thumbnailImage?.imageUrl
        entity.isAIGenerated = model.thumbnailImage?.isAiGenerated ?? false
